...
export default function ComponentsPage() {
  ...
  async function handleSave(c: Component) {
    ...
    try {
      if (editing) {
        // Use the DB uuid (c.id) for update
        await componentsApi.update(c.id, {
          ...c,
          dependency_mode: c.dependency_mode === "wait" ? "wait_all" : c.dependency_mode,
          dependency_transfer_time_minutes: c.transfer_time_minutes,
        })
        toast.success(`Component "${c.name}" updated`)
      } else {
        await componentsApi.create(planId, {
          ...c,
          dependency_mode: c.dependency_mode === "wait" ? "wait_all" : c.dependency_mode,
          dependency_transfer_time_minutes: c.transfer_time_minutes,
        })
  ...
  async function handleDelete() {
    ...
    try {
      await componentsApi.delete(deleteTarget)
  ...
  const filteredComponents = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (c) =>
          (c.component_id ?? c.id).toLowerCase().includes(q) ||
  ...
  if (!planId || !plan) {
    return (
      <div className="flex flex-col h-full">
        <AppHeader title="Components" description="Manage parts to be produced" />
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-4 md:p-6 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="flex flex-wrap items-end gap-4 p-3 rounded-lg border bg-muted/30">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input type="date" value={startDateFilter} onChange={(e) => setStartDateFilter(e.target.value)} className="w-36 h-8" />
          </div>

          <div className="flex flex-col gap-1.5">
            <Label className="text-xs">Due Date Until</Label>
            <Input type="date" value={dueDateFilter} onChange={(e) => setDueDateFilter(e.target.value)} className="w-36 h-8" />
          </div>

          {(searchQuery || statusFilter !== "all" || startDateFilter || dueDateFilter) && (
            <Button variant="ghost" size="sm" onClick={() => { setSearchQuery(""); setStatusFilter("all"); setStartDateFilter(""); setDueDateFilter("") }} className="h-8">
              Clear filters
            </Button>
          )}
        </div>
              ...
              {filteredComponents.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={13} className="text-center py-8 text-muted-foreground">
                    {components.length === 0
                      ? "No components yet. Add one or import from Excel."
                ...
                filteredComponents.map((c) => {
                  ...
                  const displayId = c.component_id ?? c.id
                  return (
                    <TableRow key={c.id} className={isComplete ? "bg-emerald-500/10" : ""}>
                      <TableCell className="font-mono text-sm">
                        {displayId}
                        {isComplete && (
                          <Badge variant="default" className="ml-2 text-[10px] bg-emerald-600">Done</Badge>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="text-right">{c.lead_time_days}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {c.order_code ? (
                          <Badge variant="outline" className="text-xs font-mono">{c.order_code}</Badge>
                        ) : (
                          <span className="text-xs text-muted-foreground">None</span>
                        )}
                      </TableCell>
      ...
      <ComponentFormDialog
        ...
        component={editing}
        existingIds={components.map((c) => c.component_id ?? c.id)}
